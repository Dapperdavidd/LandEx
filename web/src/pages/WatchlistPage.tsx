...
export function WatchlistPage() {
  ...
  useEffect(() => {
    ...
    apiRequest<WatchlistSummary[]>('/watchlists', { headers, signal: controller.signal }).then(async (lists) => {
      ...
      const properties = await Promise.all(propertyItems.map(async (item) => {
        const [property, history] = await Promise.all([
          apiRequest<PropertyListItem>(`/properties/${item.property_id}`, { signal: controller.signal }),
          apiRequest<PropertyHistoryPoint[]>(`/properties/${item.property_id}/history?limit=60`, { signal: controller.signal }),
        ])
        return { itemId: item.id, property, history }
      }))
...
function WatchlistSparkline({ history }: { history: PropertyHistoryPoint[] }) {
  const values = [...history].reverse().map((point) => Number(point.asking_price ?? point.estimated_value)).filter(Number.isFinite)
  if (values.length < 2) return <span className="watchlist-sparkline--empty" title="At least two verified observations are required">Awaiting data</span>
  const min = Math.min(...values); const spread = Math.max(...values) - min || 1
  const path = values.map((value, index) => `${index ? 'L' : 'M'} ${(index / (values.length - 1)) * 100} ${28 - ((value - min) / spread) * 24}`).join(' ')
  const rising = values.at(-1)! >= values[0]!
  return <svg className={`watchlist-sparkline${rising ? '' : ' is-negative'}`} viewBox="0 0 100 32" preserveAspectRatio="none" role="img" aria-label={`Verified property price trend from ${history.at(-1)?.observed_on} to ${history[0]?.observed_on}`}><path d={path} /></svg>
}
