...
import type { PropertyListItem, SavedSearch, WatchlistDetail, WatchlistSummary } from '../types/property'
...
function ListedInstrumentUniverse() {
  ...
  useEffect(() => { if (!selected) return; const controller = new AbortController(); const id = selected.id; apiRequest<InstrumentDetail>(`/instruments/${id}?history_limit=120`, { signal: controller.signal }).then((value) => setLoadedDetail({ id, value })).catch(() => undefined); return () => controller.abort() }, [selected])
  ...
  const detail = loadedDetail && loadedDetail.id === selected?.id ? loadedDetail.value : null
  const latestPoint = detail?.history[0], oldestPoint = detail?.history.at(-1)
  const periodChange = latestPoint && oldestPoint && latestPoint !== oldestPoint ? ((Number(latestPoint.value) / Number(oldestPoint.value) - 1) * 100) : null
  return <section className="instrument-universe listed-universe"><div className="instrument-universe__intro"><div><DataLabel>SEC EDGAR / listed real estate</DataLabel><h2>Public-market real estate,<br />without invented quotes.</h2></div><p>Issuer identity comes from the SEC. Development quotes appear only when a permitted prototype feed has supplied real closes; otherwise price and history stay unavailable. Real-money investing remains disabled.</p></div><div className="listed-terminal"><div className="listed-ledger"><div className="listed-ledger__head"><span>Issuer</span><span>Symbol</span><span>Price</span><span>Exchange</span></div>{instruments.data.items.map((instrument) => <button className={instrument.id === selected?.id ? 'is-selected' : ''} type="button" key={instrument.id} onClick={() => setSelectedId(instrument.id)}><div><strong>{instrument.name}</strong><small>LISTED SECURITY · {instrument.status === 'paper_tradeable' ? 'DEVELOPMENT PAPER' : 'RESEARCH ONLY'} · REAL MONEY OFF</small></div><b>{instrument.symbol ?? '—'}</b><span>{instrument.value ? formatMoney(instrument.value, instrument.currency) : '—'}</span><span>{instrument.exchange ?? '—'}</span></button>)}</div><aside className="instrument-inspector listed-inspector">{selected && <><DataLabel>Selected listed instrument</DataLabel><h3>{selected.name}</h3><p>{selected.symbol} / {selected.exchange} · {selected.status === 'paper_tradeable' ? 'Development paper instrument' : 'Identity research only'}</p><div className="instrument-inspector__quote"><strong>{selected.value ? formatMoney(selected.value, selected.currency) : 'Quote unavailable'}</strong>{periodChange !== null && <span className={periodChange >= 0 ? 'signal--positive' : 'signal--negative'}>{formatPercent(String(periodChange))}<small>Available-series change</small></span>}</div><dl><div><dt>Identity</dt><dd>SEC verified</dd></div><div><dt>Quotes</dt><dd>{selected.value ? 'Development EOD' : 'Unavailable'}</dd></div><div><dt>Paper mode</dt><dd>{selected.status === 'paper_tradeable' ? 'Enabled' : 'Disabled'}</dd></div><div><dt>Real money</dt><dd>Disabled</dd></div></dl><div className="instrument-inspector__chart"><DataLabel>{detail?.history.length ? `${detail.history.length} verified daily closes` : 'Price history unavailable'}</DataLabel>{detail?.history.length ? <InstrumentChart detail={detail} /> : <p>A permitted quote feed has not supplied this instrument yet.</p>}</div><div className="listed-inspector__actions">{selected.status === 'paper_tradeable' && <Link to={`/paper-invest?instrument=${selected.id}`}>Paper invest</Link>}<WatchInstrumentButton instrumentId={selected.id} />{selected.source_url && <a href={selected.source_url} target="_blank" rel="noreferrer">Verify SEC identity ↗</a>}</div><p className="listed-universe__truth">Prototype quote data, when present, is development-only and not licensed for production redistribution. This is not a recommendation or real-money offering.</p></>}</aside></div><p className="listed-universe__truth">Showing {instruments.data.items.length} of {instruments.data.total} conservative matches whose SEC issuer names explicitly identify them as REITs. This is not a complete U.S. REIT universe.</p></section>
}
