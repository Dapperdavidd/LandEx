...
export function MarketsPage() {
  ...
  useEffect(() => { if (!marketId) return; const controller = new AbortController(); Promise.all([apiRequest<MarketDetail>(`/markets/${marketId}?history_limit=60`, { signal: controller.signal }), apiRequest<PropertyScore>(`/markets/${marketId}/score`, { signal: controller.signal })]).then(([detail, score]) => { setSelectedDetail(detail); setSelectedScore(score) }).catch(() => { setSelectedDetail(null); setSelectedScore(null) }); return () => controller.abort() }, [marketId])
